...
# Public change log for the saksbehandling axis (issue #55 §4): a CC-BY data file
# baked inline so the provenance of every added/changed record — its method,
# source, and date — is visible, not hidden. Each entry:
# {date, entity?, domain?, method, source?, summary}. Empty list = nothing logged
# yet (honest empty state, never a fabricated entry).
SAK_ENDRINGSLOGG = os.path.join(ROOT, "data", "saksbehandling-endringslogg.json")
...
# tiers, both requiring a source (rule 1); anything else stays *utledet* (vendor
# inference) or *ikke kartlagt* — never "bekreftet".
#   offentlig-journal — a postjournal saksnummer/journalpost (resolvable on
#     einnsyn.no / norske-postlister.no) or a published databehandleravtale /
#     Doffin award URL. MUST carry a resolvable http(s) URL → clickable evidence.
#   innsyn-pa-fil — a real innsyn response the operator verified, held on file
#     (offered on request), not necessarily a public URL. Medium.
# The Norwegian tier labels are rendered client-side (see the SAK_TIER_NO map in
# the page JS) so the data stays language-neutral — only the tier token is baked.

# The innsyn-portal host fingerprint (issue #55 §3): the third-party host a body's
# website loads its public postjournal from is an INDEPENDENT signal of which
# sakarkiv vendor it runs. Substring match against the web axis' third_parties.
# (We already saw innsynpluss.onacos.no on Larvik's web axis — the signal exists.)
PORTAL_FINGERPRINTS = [
    ("onacos.no", "Acos"),
    ("acossky.no", "Acos"),
...
# Sikri, "Tietoevry Public 360" -> Tietoevry, "ePhorte (legacy)" -> ePhorte).
_VENDOR_FAMILIES = ("Acos", "Sikri", "Tietoevry", "ePhorte")


def vendor_family(vendor):
    """The fingerprint family a CSV vendor name belongs to, or None (e.g.
    Documaster has no innsyn-portal fingerprint)."""
    v = (vendor or "").lower()
    for fam in _VENDOR_FAMILIES:
        if fam.lower() in v:
            return fam
    return None


def portal_fingerprint(web):
    """The innsyn-portal fingerprint from an entity's web-axis record, or None.
    Returns {host, vendor} where vendor is the family the portal host implies."""
    if not web:
        return None
    for t in web.get("third_parties") or []:
        dom = (t.get("domain") or "").lower()
        for needle, fam in PORTAL_FINGERPRINTS:
            if needle in dom:
                return {"host": t.get("domain"), "vendor": fam}
    return None


def crosscheck_vendor(vendor, web):
    """Corroborate a per-body vendor claim against the innsyn-portal fingerprint
    (issue #55 §3). Returns None when there is no portal signal; otherwise
    {host, portal_vendor, agree} where agree is True (two independent sources),
    False (conflict — flag for review, do NOT publish), or None (portal present
    but the vendor has no fingerprint family to compare)."""
    fp = portal_fingerprint(web)
    if not fp:
        return None
    fam = vendor_family(vendor)
    return {"host": fp["host"], "portal_vendor": fp["vendor"],
            "agree": (fam == fp["vendor"]) if fam else None}
...
def _hosting_source_tier(row):
    """Which confirmation tier a row's hosting earns, plus its source. #55: a
    'bekreftet' verdict REQUIRES a re-checkable source. offentlig-journal is the
    highest tier and MUST carry a resolvable http(s) URL (else it can't be
    re-checked → no bekreftet); innsyn-pa-fil is an innsyn answer held on file
    (a reference, not a public URL). Returns (tier, source) or (None, source)."""
    st = (row.get("hosting_source_type") or "").strip()
    src = (row.get("hosting_source") or "").strip()
    # Back-compat: legacy rows flagged hosting_method=innsyn-foi with no source_type.
    if not st and (row.get("hosting_method") or "") == "innsyn-foi" and src:
        st = "innsyn-pa-fil"
    if st == "offentlig-journal" and src.startswith(("http://", "https://")):
        return "offentlig-journal", src
    if st == "innsyn-pa-fil" and src:
        return "innsyn-pa-fil", src
    return None, src
...
def resolve_saksbehandling(row, vendor_table=VENDOR_HOSTING):
    ...
    without a source URL is NOT a verdict (rule 1) → None. Hosting is *bekreftet*
    only when it carries a re-checkable source (hosting_source_type ∈
    {offentlig-journal, innsyn-pa-fil}, issue #55); otherwise it falls back to the
    vendor→hosting table (an *inferred*, flagged jurisdiction), or None when the
    vendor isn't in the table."""
    ...
    tier, src = _hosting_source_tier(row)
    if tier:
        rec["hosting"] = {
            "confirmed": True, "method": row.get("hosting_method") or "innsyn-foi",
            "tier": tier, "clickable": tier == "offentlig-journal",
            ...
            "source": src, "date": row.get("hosting_date") or None,
        }
    else:
        t = vendor_table.get(row["vendor"])
        rec["hosting"] = {
            "confirmed": False, "method": "vendor-table", "tier": "utledet",
            "clickable": False,
...
def attach_saksbehandling(entities, sak_index):
    ...
    email domain, and run the innsyn-portal cross-check (#55 §3) against the entity's
    already-joined web axis. Bodies with no intake row carry None — the axis stays
    distinct from the email verdict, and 'ikke kartlagt ennå' is an honest empty
    state."""
    out = []
    for e in entities:
        rec = (resolve_saksbehandling(sak_index[e.get("domain")])
        ...
        if rec:
            ...
            rec["conflict"] = bool(xc and xc["agree"] is False)
        out.append({**e, "saksbehandling": rec})
    return out
...
def sak_aggregate(categories):
    ...
    confirmed = [e for e in mapped
                 if (e["saksbehandling"].get("hosting") or {}).get("confirmed")
                 and not e["saksbehandling"].get("conflict")]
...
def render_html(meta, categories, history, trend, corrections=None, sak_log=None):
    ...
    payload = {
        ...
        # The saksbehandling axis change log (issue #55 §4) rides along inline;
        # empty list when nothing has been logged yet.
        "sakEndringslogg": sak_log or [],
...
def build_html(data, history, trend, stat=None, web=None, seeded=None,
               corrections=None, sak=None, sak_log=None):
    ...
    return render_html(data["meta"], build_categories(data, stat, web, seeded, sak),
                       history, trend, corrections, sak_log)
...
def main():
    ...
    html = render_html(data["meta"], categories, history, trend, corrections, sak_log)
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .sak-onfile{color:var(--muted)}
  .sak-2src{font-size:var(--text-xs);font-weight:600;color:var(--green,#2e7d32)}
  ...
  .changelog .cl-meta{font-size:var(--text-xs)}
  .changelog .cl-meta a{color:var(--accent)}
  ...
  <section id="view-om" class="hidden">
    ...
    <div class="panel">
      ...
      <p><b>Saksbehandling / arkiv (tredje akse).</b> Hvilket sak-/arkivsystem
        ...
        per organ mot en <b>re-sjekkbar kilde</b>: en <b>offentlig journal</b>
        (postjournal/journalpost på einnsyn.no eller norske-postlister.no, en
        databehandleravtale eller en Doffin-tildeling — høyest, med klikkbar
        lenke) eller et <b>innsynssvar på fil</b> (verifisert av operatøren,
        tilgjengelig på forespørsel). Ingen jurisdiksjon påstås per organ fra
        tabellen uten «utledet»-flagget; <b>ingen «bekreftet» uten en kilde en
        skeptiker kan etterprøve</b>. Der en leverandørpåstand og organets
        innsynsportal-fingeravtrykk peker på samme leverandør, merkes den
        «bekreftet av to uavhengige kilder»; ved konflikt flagges den og
        publiseres ikke. <span class="sak-agg"><!--__SAK_AGGREGATE__--></span></p>
      <p style="margin-bottom:0"><b>Endringslogg — saksbehandlingsaksen</b> (hva
        som er lagt til eller endret, med metode, kilde og dato):</p>
      <ul class="changelog" id="sak-endringslogg"></ul>
...
(function(){
  ...
  function renderSaksarkiv(k){
    ...
    var head = '<h2>Saksbehandling / arkiv</h2>'+
      '<p style="font-size:13px;color:var(--muted);margin:-6px 0 12px">En '+
        ...
        'fra leverandøren (flagget) eller <b>bekreftet</b> mot en re-sjekkbar kilde '+
        '(offentlig journal eller innsyn på fil). Ingen «bekreftet» uten kilde en '+
        'skeptiker kan etterprøve. Påvirker ikke e-postverdiktet over.</p>';
    if(!s){
    ...
    // Cross-check conflict (#55 §3): the vendor claim disagrees with the innsyn-
    // portal fingerprint → flagged for operator review, verdict NOT published.
    if(s.conflict){
      var cx = s.crosscheck || {};
      return head+
        '<div class="facts">'+fact("Saksarkiv", "Flagget for gjennomgang", "red")+'</div>'+
        '<p style="font-size:13px;color:var(--muted);margin-top:10px">Leverandør'+
          'påstanden (<b>'+esc(s.vendor)+'</b>) strider mot innsynsportal-finger'+
          'avtrykket (<b>'+esc(cx.host||"")+'</b> → '+esc(cx.portal_vendor||"")+'). '+
          'Verdiktet er <b>ikke publisert</b> før en operatør har gått gjennom det.</p>'+
        krevInnsynSak(k);
    }
    // Corroboration marker (#55 §3): vendor claim AND portal fingerprint agree.
    var xc = s.crosscheck;
    var twoSrc = (xc && xc.agree === true)
    ...
    if(!h){
      ...
      // Tiered badge (#55 §2). The offentlig-journal tier renders a CLICKABLE
      // evidence link (the postjournal/document URL) so any reader re-verifies;
      // innsyn på fil is held on file, offered on request (no public URL).
      var SAK_TIER_NO = {"offentlig-journal": "bekreftet – offentlig journal",
                         "innsyn-pa-fil": "bekreftet – innsyn på fil"};
      var badge = '<b>'+esc(SAK_TIER_NO[h.tier] || "bekreftet")+
        (h.date? ' '+esc(noDate(h.date)):"")+'</b>';
      var ev = h.clickable
        ? ' <span class="sak-src">'+sakSrc(h.source, h.date, "offentlig journal")+'</span>'
        : ' <span class="sak-src sak-onfile">innsynssvar på fil'+
            (h.source? ' — '+esc(h.source) : "")+' (tilgjengelig på forespørsel)</span>';
      hostFact = fact("Hosting (jurisdiksjon)",
        esc(h.jurisdiction)+' — '+badge+ev, jurCls(h.jurisdiction));
    } else {
      var src = h.source
  ...
  // Saksbehandling-axis change log (issue #55 §4): the same open-provenance
  // pattern as the corrections log, but per-axis — every row shows its method,
  // source and date. Honest empty state until the first entry is logged.
  function renderSakEndringslogg(){
    var el = document.getElementById("sak-endringslogg");
    if(!el) return;
    var c = DB.sakEndringslogg || [];
    if(!c.length){
      el.innerHTML = '<li class="cl-empty">Ingen endringer logget ennå. Loggen er '+
        'offentlig og oppdateres her når et organ legges til eller endres.</li>';
      return;
    }
    el.innerHTML = c.map(function(r){
      var who = r.entity ? '<span class="cl-who">'+esc(r.entity)+'</span> · ' : "";
      var meta = [];
      if(r.method) meta.push("metode: "+esc(r.method));
      if(r.source) meta.push(/^https?:\/\//.test(r.source)
        ? 'kilde: <a href="'+esc(r.source)+'" target="_blank" rel="noopener">lenke</a>'
        : "kilde: "+esc(r.source));
      var tail = meta.length ? ' <span class="cl-meta">('+meta.join(" · ")+')</span>' : "";
      return '<li class="cl-row"><span class="cl-date">'+esc(noDate(r.date))+
        '</span><span class="cl-text">'+who+esc(r.summary)+tail+'</span></li>';
    }).join("");
  }
  ...
  renderCorrections(); renderSakEndringslogg(); wireFinder(); wireExplore(); wireLeague(); wireMapToggle(); route();
})();
</script>
</body>
</html>
