...
# Issue #61: the generated portal-fingerprint dataset merged UNDER the human CSV.
AUTO = {
    # A body the human CSV has NOT mapped — filled by the fingerprint.
    "trondheim.kommune.no": {
        "domain": "trondheim.kommune.no", "kommune": "Trondheim",
        "vendor": "Acos WebSak", "vendor_method": "portal-fingerprint",
        "vendor_source": "https://innsyn.acossky.no", "vendor_date": "2026-07-02"},
    # A body the human CSV ALSO maps (Oslo, in SAK) — manual must win.
    "oslo.kommune.no": {
        "domain": "oslo.kommune.no", "kommune": "Oslo",
        "vendor": "Sikri Elements", "vendor_method": "portal-fingerprint",
        "vendor_source": "https://prod.elementscloud.no", "vendor_date": "2026-07-02"},
}


class SaksbehandlingAutoMerge(unittest.TestCase):
    """#61: the auto (portal-fingerprint) index fills gaps UNDER the human CSV."""

    def test_manual_wins_over_auto(self):
        merged = build.merge_saksbehandling(SAK, AUTO)
        # Oslo is in both — the human CSV row (Tietoevry) must win, not auto Sikri.
        self.assertEqual(merged["oslo.kommune.no"]["vendor"], "Tietoevry Public 360")

    def test_auto_fills_a_gap_the_csv_did_not_cover(self):
        merged = build.merge_saksbehandling(SAK, AUTO)
        self.assertEqual(merged["trondheim.kommune.no"]["vendor"], "Acos WebSak")

    def test_auto_record_is_vendor_only_never_bekreftet_hosting(self):
        # A fingerprint identifies the VENDOR; hosting stays utledet/Uavklart —
        # NEVER confirmed (issue #61 §3).
        rec = build.resolve_saksbehandling(AUTO["trondheim.kommune.no"])
        self.assertEqual(rec["vendor"], "Acos WebSak")
        self.assertFalse(rec["hosting"]["confirmed"])
        self.assertEqual(rec["hosting"]["tier"], "utledet")
        self.assertEqual(rec["hosting"]["jurisdiction"], "Uavklart")

    def test_auto_record_earns_no_two_source_badge(self):
        # The vendor came FROM the portal — cross-checking it against the same
        # portal is not two INDEPENDENT sources, so no corroboration badge.
        web = _web_with_portal("trondheim.kommune.no", "innsyn.acossky.no")
        entities = [{"domain": "trondheim.kommune.no", "web": web}]
        out = build.attach_saksbehandling(entities, AUTO)
        self.assertIsNone(out[0]["saksbehandling"]["crosscheck"])

    def test_aggregate_counts_auto_as_mapped_but_not_confirmed(self):
        merged = build.merge_saksbehandling({}, AUTO)
        entities = [{"domain": d, "web": None} for d in
                    ("trondheim.kommune.no", "oslo.kommune.no")]
        out = build.attach_saksbehandling(entities, merged)
        agg = build.sak_aggregate([{"entities": out}])
        self.assertEqual(agg["mapped"], 2)       # both fingerprinted
        self.assertEqual(agg["confirmed"], 0)    # a fingerprint is never confirmed

    def test_loader_reads_generated_dataset(self):
        # The real generated file loads into a domain index (regenerated by the
        # zero-cost probe pass, committed alongside).
        idx = build.load_saksbehandling_auto()
        self.assertTrue(idx)                     # non-empty after the probe ran
        for row in idx.values():
            self.assertEqual(row["vendor_method"], "portal-fingerprint")
