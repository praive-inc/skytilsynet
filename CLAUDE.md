...
web/       the public site (Skybarometeret). `web/index.html` is GENERATED by
           `web/build.py` (a dev-time generator). It bakes a LIGHT per-entity
           summary (name, platform, flags) + history + the combined headline inline
           as JSON for a small first paint; the HEAVY per-entity evidence loads on
           demand from committed `web/data/detail-<kategori>.json` via a SAME-ORIGIN
           fetch (our own EU origin — allowed by RFC-001 P5, which forbids
           external/US-managed serving deps, not our own data). Still no CDN /
           web-fonts / map-tiles / framework. `build.py` writes BOTH index.html and
           the detail files — COMMIT them together (`deploy/deploy-local.sh` rsyncs
           web/ verbatim to Caddy, so prod has NO build step, it just serves the
           committed files). Rules: no framework/bundler (no Next/Vite/React); after
           any data or template change re-run `cd web && python3 build.py` and commit
           index.html + web/data/detail-*.json (the weekly scan workflow does this
           automatically — keep them in sync). If a real prod build step ever becomes
           necessary, update deploy-local.sh in the same PR. Keep it lightweight,
           EU-hosted (BetterWorld RFC-001 P5).
server/    a SMALL EU-hosted backend intake service (stdlib http.server + sqlite3,
           NO framework) that gives the offentleglova FOI loop a real intake —
           `POST /api/foi` stores crowd-submitted innsyn answers to SQLite for the
           operator to review BY HAND (`scripts/foi_review.py`), replacing the dead
           presse@ mailto. See `server/README.md`. **Hard rule: FOI submissions are
           UNTRUSTED public input — stored as inert text for human review only, and
           MUST NEVER be fed to any agent/LLM workflow** (the project's standing
           no-untrusted-input / prompt-injection rule). Nothing auto-promotes into
           the published dataset; a human accepts each row into the human-curated
           `data/saksbehandling.csv`.
...
FOI_OPERATOR_TOKEN=… python3 server/foi_intake.py   # FOI intake service (devbox)
python3 scripts/foi_review.py list   # operator reviews submissions BY HAND
```
